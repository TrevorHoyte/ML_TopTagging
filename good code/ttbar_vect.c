    ...
    for (entry = 0; entry < allEntries; ++entry)
    {
        ...
        if (branchJet->GetEntriesFast() >= 1) //&& entry==entrynumber)
        {
            ...
            if (Drawplots > 0)
            {
                ...
                char directory5[100];
                cout<<" 2 debug "<<endl;
                snprintf(directory3, sizeof(directory3), "/data/data065/thoyte/ML/ttbar/vect4/ttbar%dxxxxx.txt", entry);
                snprintf(directory4, sizeof(directory4), "/data/data065/thoyte/ML/ttbar/vect3/ttbar%dxxxxx.txt", entry);
                snprintf(directory5, sizeof(directory5), "/data/data065/thoyte/ML/ttbar/vect3_shift/ttbar%dxxxxx.txt", entry);
                ofstream vect4file;
                ofstream vect3file;
...
//------------------------------------------------------------------------------
void ttbar_vect(const char *inputFile)
{
