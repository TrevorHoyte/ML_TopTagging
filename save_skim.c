 ...
 int AnalyseEvents(ExRootTreeReader *treeReader, TestPlots *plots, int entrynumber)
{
  ...
  TLorentzVector momentum,jet_momentum,top_momentum;
  Float_t Eem, Ehad;
  Bool_t skip;
 ...
 //{
    ...
    if(branchJet->GetEntriesFast() >= 1 )//&& entry==entrynumber) 
    {
      ...
      jet = (Jet*) branchJet->At(0);//fix this 
        cout << jet->Eta << " the jets eta, "<<jet->Phi<<" the jets phi, " << "event #"<< entry <<endl; 
            ...
            if(object->IsA() == Tower::Class())
                {
                    ...
                    tower=(Tower*) jet->Constituents.At(hardest2);
                    phi_2 = tower->Phi;
                    rap_2 =tower->Eta;
                    
                    //Translation: shift hardest to center 
                    tower= (Tower*) jet->Constituents.At(hardest1);
...
else{cout<<"nothing to save"<<endl;}
 
}
//------------------------------------------------------------------------------
void save_skim(const char *inputFile)
{
